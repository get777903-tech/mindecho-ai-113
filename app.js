...
function handleCustDevSubmit(e) {
  ...
  for (let idx = 0; idx < 3; idx++) {
    const input = document.getElementById(`cd-input-${idx}`);
    if (input) answers.push(input.value);
  }
  ...
  const lang = appState.lang || 'ru';
  let thankMsg = "🎉 Спасибо за ваши ответы! Вам предоставлен приоритетный VIP-доступ.";
  if (lang === 'en') thankMsg = "🎉 Thank you for your answers! You have been granted priority VIP access.";
  if (lang === 'he') thankMsg = "🎉 תודה על תשובותיך! הוענקה לך גישת VIP בעדיפות.";
